...
class ResourcePersistableTest
    extends FixtureAnyFunSpec
    with TempDirectoryFixture
    with GivenWhenThen {

  val persistableInstance: Persistable.Aux[BundledResource] =
    ResourcePersistable.apply
  ...
  describe("Persisting a picture from the resources folder") {
    it("should create a file named by its content") { (tmpDir: Path) =>
      ...
      val elementToPersist: BundledResource =
        BundledResource(
          "res/GammaMtau.png",
          Paths.get("test", "path", "picture.png"),
          getClass()
        )
      ...
      val resourceBytes = Files.readAllBytes(
        Paths.get(getClass.getResource("res/GammaMtau.png").toURI)
      )
      ...
      persistableInstance.persistOverwriting(
